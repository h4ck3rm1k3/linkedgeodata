...
CREATE OR REPLACE VIEW way_tags AS SELECT a.id node_id, (each(a.tags)).key k, (each(a.tags)).value v FROM ways a;
CREATE OR REPLACE VIEW relation_tags AS SELECT a.id node_id, (each(a.tags)).key k, (each(a.tags)).value v FROM relations a;
