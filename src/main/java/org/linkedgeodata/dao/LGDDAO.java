...
public class LGDDAO
{
	...
	public static MultiMap<Long, Tag> getTags(Connection conn, String type, Collection<Long> ids, String tagFilterStr)
		throws SQLException
	{
	 ...
	 * @return
	 */
	public Collection<Long> getNodesWithinRect(
	...
	public Collection<Long> getWaysWithinRect(
			RectangularShape rect,
			String tagFilter,
			Long offset,
			Integer limit
	...
	public Collection<Long> getNodesWithinRadius(
			Point2D point,
			double radius,
			String tagFilter,
			Integer limit,
	...
	public Collection<Long> getWaysWithinRadius(
			Point2D point,
			double radius,
			String tagFilter,
			Integer limit,
	...
	{				
		String spatialCondition = "ST_DWithin(x.geom, " + 
				LGDQueries.buildPoint(point.getY(), point.getX()) + "::geography, " +
				radius + ", true) ";
	...
	{				
		String spatialCondition = "ST_DWithin(x.linestring, " + 
				LGDQueries.buildPoint(point.getY(), point.getX()) + "::geography, " +
				radius + ", true) ";
	...
	public static String buildGetEntitiesWithinX(String tableName, String spatialCondition, String tagFilter, Integer limit, Long offset)
	{
		...
		if(tagFilter != null)
			result += "JOIN " + tableName + "_tags ON (" + tableName + "_id = id) ";
		
		result += "WHERE " + spatialCondition + " ";
		
		if(tagFilter != null)
			result += "AND " + tagFilter + " ";
