	...
	extends AbstractDAO
{
		...
		throws Exception
	{
		...
		try {
			...
			NodePositionDAO dao = new NodePositionDAO();
	...
	public Map<Long, Point2D> lookup(Iterable<Long> ids)
		throws SQLException
	{
		String sql = "SELECT node_id, latitude, longitude FROM node_position WHERE node_id IN (" + StringUtil.implode(",", ids)+ ")"; 
		...
		throws SQLException
	{
		...
		if(!inserts.isEmpty()) {
			String sql = "INSERT INTO node_position(node_id, latitude, longitude) VALUES " + mapToSQL(inserts);
		...
		for(Map.Entry<Long, Point2D> entry : updates.entrySet()) {
			String sql = "UPDATE node_position SET latitude = " + entry.getValue().getY() + ", longitude = " + entry.getValue().getX() + " WHERE node_id = " + entry.getKey();   
			SQLUtil.execute(conn, sql, Void.class);
		}
		...
		throws SQLException
	{
		...
		String sql = "DELETE FROM node_position WHERE node_id IN (" + idStr + ")"; 
		SQLUtil.execute(conn, sql, Void.class);
	}
